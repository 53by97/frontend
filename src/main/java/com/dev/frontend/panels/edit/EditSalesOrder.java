...
import com.dev.frontend.intg.models.OrderItem;
...
public class EditSalesOrder extends EditContentPanel {
	...
	private void addOrderItems() {
		// TODO Not implemented correctly from frontend side
	}
	...
	public boolean bindToGUI(Object o) {
		...
		txtOrderNum.setText(String.valueOf(order.getOrderId()));
		txtTotalPrice.setText(order.getTotalPrice().toString());
		txtCustomer.setSelectedItem(order.getCustomer().getCustId());
		// txtQuantity.setText(String.valueOf(order.ge));
		// txtProduct.setText(String.valueOf(order.ge));
	...
	public Object guiToObject() {
		...
		// private JComboBox<ComboBoxItem> txtCustomer = new
		// JComboBox<ComboBoxItem>();
		// private JComboBox<ComboBoxItem> txtProduct = new
		// JComboBox<ComboBoxItem>();

		SalesOrder salesOrder = new SalesOrder();
		salesOrder.setOrderId(Integer.valueOf(txtOrderNum.getText()));
		salesOrder.setTotalPrice(BigDecimal.valueOf(Double
				.valueOf(txtTotalPrice.getText())));

		Customer customer = new Customer();
		String custId = txtCustomer.getSelectedItem().toString();
		custId = custId.substring(custId.indexOf('(') + 1, custId.indexOf(')'));
		customer.setCustId(Integer.valueOf(custId));
		salesOrder.setCustomer(customer);

		OrderItem orderItem = new OrderItem();
		Product product = new Product();
		String prodId = txtProduct.getSelectedItem().toString();
		prodId = prodId.substring(prodId.indexOf('(') + 1, prodId.indexOf(')'));
		product.setProdId(Integer.valueOf(prodId));
		orderItem.setProduct(product);

		orderItem.setQuantity(Integer.valueOf(txtQuantity.getText()));
		orderItem.setTotalPrice(BigDecimal.valueOf(Double.valueOf(txtTotalPrice
				.getText())));
